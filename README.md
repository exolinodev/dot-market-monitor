...
Der Collector-Job besteht aus Checkout des aktuellen `main`, Python-/pip-Cache, Installation der Test-Abhängigkeiten, Warten bis zur Grenze, Datensammlung und Publikation über einen SHA-gebundenen Daten-PR (`scripts/promote_data.py`). Vor der Publikation laufen einmal auf dem committeten Kandidaten die Archivwächter (Intraday, Ledger-Replay, Oracle-Archiv), die Snapshot-Validierung und die fokussierten Runtime-Tests, im vollen Lauf zusätzlich die Scheduler-Tests; die gesamte pytest-Suite läuft im Test-Workflow bei Code-Pushes auf `main`, nicht im Datenpfad. Die harte Schema-/Plausibilitätsprüfung bleibt zusätzlich direkt in `src/main.py` vor dem Schreiben enthalten. `contents: write` und eine gemeinsame Concurrency-Gruppe erlauben geordnete Daten-Updates. Die festgelegten Output-/State-Dateien einschliesslich der neuen Beobachtungshistorie werden gestaged; `git diff --cached --quiet` verhindert Commits ohne Änderungen.

Der separate Testworkflow läuft bei Pushes auf `main` mit Änderungen an Code, Konfiguration, Schemas, Tests, Workflows, Prompt oder dem unveränderlichen Oracle-Archiv sowie bei jedem Pull Request ausser reinen `data/**`-Änderungen; Oracle-Einreichungen unter `data/oracle/submissions/**` bleiben abgedeckt. Die Daten-PRs des vertrauenswürdigen Producers tragen ihren eigenen `test`-Check aus dem Producer-Lauf. Er installiert zusätzlich die gepinnten Test-Abhängigkeiten aus `tests/requirements.txt` und prüft den Cloudflare-Starter mit `node --test tests/scheduler.test.mjs`. Node und Wrangler werden nicht im stündlichen Collector installiert.
