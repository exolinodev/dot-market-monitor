...
**v4 seit 21.09.2026 aktiv: Cloudflare dispatcht bei :58/:13/:28/:43 (zwei Minuten Vorlauf), der Runner wartet bis Grenze +8 s; :00 liefert den vollen Snapshot, :15/:30/:45 einen Light-Lauf; GitHub dient bei :03/:18/:33/:48 als Ersatz. Der 48-h-Nachweis steht noch aus.** Frische Daten und laufende Collector verhindern weitere Starts. Der Ersatzjob überspringt bei aktuellen Daten Python-Setup, Dependencies und Sammlung. Auch verspätete Prüfungen können fehlende Daten nachholen. Details: [Phase 1](docs/ORACLE_V4_PHASE1.md); bisheriger Betrieb: [Cloudflare-Starter](docs/CLOUDFLARE_SCHEDULER.md).
...
Der Collector-Job besteht aus Checkout des aktuellen `main`, Python-/pip-Cache, Installation der Test-Abhängigkeiten, Warten bis zur Grenze, Datensammlung und Publikation über einen SHA-gebundenen Daten-PR (`scripts/promote_data.py`). Vor der Publikation laufen einmal auf dem committeten Kandidaten, gleichzeitig gestartet und mit in fester Reihenfolge protokollierter Ausgabe, die Archivwächter (Intraday, Ledger-Replay, Oracle-Archiv), die Snapshot-Validierung und die fokussierten Runtime-Tests, im vollen Lauf zusätzlich die Scheduler-Tests; die gesamte pytest-Suite läuft im Test-Workflow bei Code-Pushes auf `main`, nicht im Datenpfad. Die harte Schema-/Plausibilitätsprüfung bleibt zusätzlich direkt in `src/main.py` vor dem Schreiben enthalten. `contents: write` und eine gemeinsame Concurrency-Gruppe erlauben geordnete Daten-Updates. Die festgelegten Output-/State-Dateien einschliesslich der neuen Beobachtungshistorie werden gestaged; `git diff --cached --quiet` verhindert Commits ohne Änderungen.

Der separate Testworkflow läuft nur bei Codeänderungen: bei Pushes auf `main`, die Code, Konfiguration, Schemas, Tests, Skripte, Workflows oder den Prompt berühren, sowie bei jedem Pull Request ausser reinen `data/**`-Änderungen. Reine Daten-Merges (Collector, Light, Oracle-Writer) starten ihn seit dem 21.09.2026 nicht mehr; ihre Prüfung ist der Producer-Lauf vor dem geschützten Merge, der seinen eigenen `test`-Check trägt. Oracle-Drafts prüft ausschliesslich der Writer. Ein seltener manueller Daten-PR (etwa eine neue Ledger-Genesis) erhält seinen `test`-Check durch einen `workflow_dispatch` von `tests.yml` auf seinem Branch. Die pytest-Suite läuft im Workflow parallel (`pytest -n auto`, `pytest-xdist`); der Workflow installiert die gepinnten Test-Abhängigkeiten aus `tests/requirements.txt` und prüft den Cloudflare-Starter mit `node --test tests/scheduler.test.mjs`. Node und Wrangler werden nicht im Collector installiert.
