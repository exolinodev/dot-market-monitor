"""Account-bound, cursor-paginated execution history from Kraken Futures demo."""
...
def capture_executions(directory, client, expected_account, since, through, *, max_pages=100):
    ...
    for index in range(max_pages):
        ...
        for element in elements:
            ...
            normalized = execution_fill(element, expected_account)
            fill_stamp = milliseconds(normalized['fillTime'])
    ...
    report = {'version': 1, 'environment': 'demo', 'source': 'execution_history',
              ...
              'pages': pages, 'fills': deduplicate_fills(fills),
...
def verify_capture(directory, expected_account, since, through):
    ...
    class Recorded:
        ...
        def history(self, endpoint, params):
            ...
            if (endpoint != 'executions' or metadata['params'] != params
    ...
    with tempfile.TemporaryDirectory(prefix='oracle-history-verify-') as temporary:
        rebuilt = capture_executions(Path(temporary) / 'capture', recorded, expected_account,
                                     since, through, max_pages=page_count)
