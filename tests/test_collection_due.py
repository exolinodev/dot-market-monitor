...
def test_chatgpt_rerun_of_scheduled_job_can_refresh_within_same_round():
    now = datetime(2026, 9, 13, 2, 57, tzinfo=timezone.utc)
    document = doc("2026-09-13T02:51:00Z")
    assert not due(document, now, "schedule", run_attempt=1)
    assert due(document, now, "schedule", run_attempt=2)
    assert not due(doc("2026-09-13T02:56:00Z"), now, "schedule", run_attempt=2)
